classdef MMD < handle
    ...
    methods (Access = private)
        function rawData = MMD_iteration(obj, V, driveCondition, targetCAx)
            ...
            % unpack car params
            m = obj.carParams.m;                % Total Mass [kg]
            PFront = obj.carParams.PFront;      % Percent Mass Front [0-1]
            WB = obj.carParams.WB;              % Wheelbase [m]
            TWf = obj.carParams.TWf;            % Trackwidth [m]
            TWr = obj.carParams.TWr;
            hCG = obj.carParams.hCG;            % CG height [m]
            CamberFront = obj.carParams.CamberFront;
            CamberRear = obj.carParams.CamberRear;
            
            Cl = obj.carParams.Cl;
            Cd = obj.carParams.Cd; 
            CoP = obj.carParams.CoP;            % front downforce distribution (%)
            rho = obj.carParams.rho;            % kg/m^3
            crossA = obj.carParams.crossA;      % m^2
            
            B_FBB = obj.carParams.B_FBB;        % Front brake bias
        
            Idx = obj.carParams.tire.Idx;                     % Moment of Inertia in x for wheel
            TirePressure = obj.carParams.tire.TirePressure;           % kPa
            Model = obj.carParams.tire.Model;
            Tire = obj.carParams.tire.Tire;

            % load cache
            SA_CG = obj.MMDCache.SA_CG;
            dSteer = obj.MMDCache.dSteer;
            coord_AllW = obj.carParams.coord_AllW;
            dSteer_Allw = obj.MMDCache.dSteer_Allw;
            ...
            for i = 1:length(dSteer)
                ...
                for j = 1:length(SA_CG)
                    ...
                    itAyBody = [];
                    itAyBody(1,1) = obj.Data.saveAyBody(i, j);
                    itAxBody = [];
                    itAxBody(1,1) = obj.Data.saveAxBody(i, j);
                    itCAyBody = [];
                    itCAyBody(1,1) = obj.Data.saveAyBody(i, j)/g;
                    itCAxBody = [];
                    itCAxBody(1,1) = obj.Data.saveAxBody(i, j)/g;
                        ...
                        % level surface stuff
                            ...
                            if abs(Ferror) > 0
                                ...
                                if driveCondition(i, j)
                                    ...
                                    motorLimFx = obj.getMotorLimit(obj.carParams, V);
    ...
    methods 
        function obj = MMD(carParams, latWTModel, motorModel, log)
            obj.carParams = carParams;
            obj.display_log = log;
            obj.calcLatWeightTransfer = latWTModel;
            obj.getMotorLimit = motorModel;

            % initialize wheel coordinates
            ...
            obj.carParams.coord_AllW = [coord_W1, coord_W2, coord_W3, coord_W4];
        end

        function obj = evaluate(obj, rangeSA, rangeSteer, velocity, mode, targetCAx)
            ...
            obj.MMDCache.SA_CG = SA_CG;
            obj.MMDCache.dSteer = dSteer;
            obj.MMDCache.V = velocity;
        
            dSteer_W1 = obj.carParams.toe_f + dSteer;
            dSteer_W2 = -obj.carParams.toe_f +dSteer;
            dSteer_W3 = obj.carParams.toe_r + dSteer.*0;
            dSteer_W4 = -obj.carParams.toe_r + dSteer.*0;
            obj.MMDCache.dSteer_Allw = [dSteer_W1, dSteer_W2, dSteer_W3, dSteer_W4];
            ...
            if mode == "free_rolling"
                obj.Data = obj.MMD_iteration(velocity);
            elseif mode == "level_surface"
                obj.Data = obj.MMD_iteration(velocity);
                driveCondition = (ones(size(rawData.saveCAxVel)).* targetCAx - rawData.saveCAxVel) > 0;
                obj.Data = obj.MMD_iteration(velocity, driveCondition, targetCAx);
            elseif mode == "const_condition"
                driveCondition = ones(size(rawData.saveCAxVel)).* targetCAx > 0;
                obj.Data = obj.MMD_iteration(velocity, driveCondition, targetCAx);
            end
        end

        function obj = reevaluate(obj, velocity, mode, targetCAx)
            if mode == "free_rolling"
                obj.Data = obj.MMD_iteration(velocity);
            elseif mode == "level_surface"
                obj.Data = obj.MMD_iteration(velocity);
        ...
        function [steadyStateCAy, obj] = getSteadyStateCAy(obj, targetCAx)
            SA_CG = obj.MMDCache.SA_CG;
            dSteer = obj.MMDCache.dSteer;
            ...
            saveMzBody = obj.Data.saveMzBody;
            saveCAyVel = obj.Data.saveCAyVel;
            saveCAxVel = obj.Data.saveCAxVel;
            ...
            obj.MMDCache.SSFy = steadyStateCAy;
        end
        
        function absoluteCAy = getAbsoluteCAy(obj)
        
            [SA_CG, dSteer] = meshgrid(obj.MMDCache.SA_CG, obj.MMDCache.dSteer);
            dataPoints = [obj.Data.saveCAyVel(1:end);
                          SA_CG(1:end);
                          dSteer(1:end);
                          obj.Data.saveMzBody(1:end)];
        ...
        function plotMMD(obj)
            dSteer = obj.MMDCache.dSteer;
            rawData = obj.Data;
            SA_CG = obj.MMDCache.SA_CG;
            V = obj.MMDCache.V;
            SSFy = obj.MMDCache.SSFy;
