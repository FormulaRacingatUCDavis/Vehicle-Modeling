...
clear
close all
delta = -20:1:20; %Change indexing back to normal  % Steer Angle [Deg]
...
R_CG_Old_Graph = linspace(-100,100,100);

hold on
for k = 20%1:length(R_CG_Old_Graph)
    for i = 10%1:length(delta)
        for j = 10%1:length(Beta)
            tic
            ...
            R_CG_Old = R_CG_Old_Graph(k); %Initial Guess
            c = 1;
             while c < 1000
                ...
                R_CG_Old = R_CG_New;
                R_CG_New_Graph_2(c) = R_CG_New;
                c = c+1;
             end
                R_CG_New_Graph(k) = R_CG_New;
                res = Inf;
        ...
        plot(Lat_Acc_Hold./9.8,Yaw_Acc_Hold, 'b')
...
figure(2)
subplot(2,1,1)
%plot(R_CG_Old_Graph,R_CG_New_Graph)
subplot(2,1,2)
plot(R_CG_New_Graph_2)
