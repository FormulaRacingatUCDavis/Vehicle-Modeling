...
Parameter.rearsteer = 1/6;
yaw_rate = 0;                       % Yaw Rate initialize
Parameter.turning_Radius = 10;                % CHECK Turning Radius (m)

load('Hoosier_LCO_16x75-10x7.mat'); % Loading Tire Data (Tire Repo);
Parameter.SlipRatio = 0.01;
Parameter.Pressure = 80;
Parameter.Inclination = 0; 
...
Parameter.I_CoG = Parameter.Mass .* 0.7^2;
...
Parameter.suspended_mass = 77;                      % Suspended Mass (kg)
Parameter.height_CG_Suspended = 0.230;              % Suspendd CoG Height (m) 
Parameter.roll_Center_Height = 0.1;                 % Roll Center Height (m) 
Parameter.roll_Stiffness_Distribution_Front = 0.60; % Percent Front Roll Stiffness (%)
Fz = zeros(1,4);
...
r_1= [Parameter.a;Parameter.tw/2;0];
r_2= [Parameter.a;-Parameter.tw/2;0];
r_3= [Parameter.b;Parameter.tw/2;0];
r_4= [Parameter.b;-Parameter.tw/2;0];

%% Acceleration Calculation
lat_Vel = tanh(beta).*long_Vel;         % Doble Check
...
deltaFz_Lateral_Front_Unsprung = (2 .* Parameter.unsprung_mass .* latAcc .*...
    Parameter.unsprung_CG_Height) ./ Parameter.tw;

deltaFz_Lateral_Rear_Unsprung = (2 .* Parameter.unsprung_mass .* latAcc .*...
    Parameter.unsprung_CG_Height) ./ Parameter.tw;
...
deltaFz_Lateral_Front_Suspended_Elastic = ((Parameter.suspended_mass .* latAcc .* (Parameter.height_CG_Suspended)...
    - Parameter.roll_Center_Height) ./ Parameter.tw ) .* Parameter.roll_Stiffness_Distribution_Front;

deltaFz_Lateral_Rear_Suspended_Elastic = ((Parameter.suspended_mass .* latAcc .* (Parameter.height_CG_Suspended)...
    - Parameter.roll_Center_Height) ./ Parameter.tw ) .* (1-Parameter.roll_Stiffness_Distribution_Front);
...
yaw_Acc = (cross(r_1,F_1) + cross(r_2,F_2) + cross(r_3,F_3)...
    + cross(r_4,F_4))./ Parameter.I_CoG;  %Shouldn't be a column vector?
yaw_Acc = yaw_Acc(3)
