...
%delta = -10:0.5:10;
delta = -2:0.5:2; %Change indexing back to normal  % Steer Angle [Deg]
Beta = -3:1:3; %change indexing back to normal    % Body Slip Angle [Deg]
...
for i = 1:length(delta)
    for j = 1:length(Beta)
        ...
        syms R_CG LA Lat_Acc_3  %Initialize symbols for symbolic solve for R_CG
        ...
        Yaw_Rate = ((Long_Vel^2 + Lat_Vel^2)^(1/2))/(R_CG);
        Omega = [0;0;Yaw_Rate];
        ...
        Lat_Acc = (Long_Vel^2+Lat_Vel^2)/R_CG;
        ...
        R_CG = (Long_Vel^2+Lat_Vel^2)/Lat_Acc_1 == R_CG;
        S = vpasolve(R_CG);
        R_CG_Calc = double(S);
        fun = matlabFunction(Lat_Acc_1);
        Fy_Function_First = matlabFunction(Fy(1));
        Fy_Function_Second = matlabFunction(Fy(2));
        Fy_Function_Third = matlabFunction(Fy(3));
        Fy_Function_Fourth = matlabFunction(Fy(4));
        Fz_Function_First = matlabFunction(F_Vert(1));
        Fz_Function_Second = matlabFunction(F_Vert(2));
        Fz_Function_Third = matlabFunction(F_Vert(3));
        Fz_Function_Fourth = matlabFunction(F_Vert(4));
    
        Acceleration = fun(R_CG_Calc);
        Fy_Matrix(1) = Fy_Function_First(R_CG_Calc);
        Fy_Matrix(2) = Fy_Function_Second(R_CG_Calc);
        Fy_Matrix(3) = Fy_Function_Third(R_CG_Calc);
        Fy_Matrix(4) = Fy_Function_Fourth(R_CG_Calc);
        Fz_Matrix(1) = Fz_Function_First(R_CG_Calc);
        Fz_Matrix(2) = Fz_Function_Second(R_CG_Calc);
        Fz_Matrix(3) = Fz_Function_Third(R_CG_Calc);
        Fz_Matrix(4) = Fz_Function_Fourth(R_CG_Calc);
    
        Lat_Acc_Hold(j) = Acceleration;
    
        %Tire Forces (Only Lateral) (Degrees or radians?)
        F_Matrix{1} = [-Fy_Matrix(1) * sind(Tire_Angle(1)); Fy_Matrix(1) * cosd(Tire_Angle(1)); Fz_Matrix(1) ];
        F_Matrix{2} = [-Fy_Matrix(2) * sind(Tire_Angle(2)); Fy_Matrix(2) * cosd(Tire_Angle(2)); Fz_Matrix(2) ];
        F_Matrix{3} = [-Fy_Matrix(3) * sind(Tire_Angle(3)); Fy_Matrix(3) * cosd(Tire_Angle(3)); Fz_Matrix(3) ];
        F_Matrix{4} = [-Fy_Matrix(4) * sind(Tire_Angle(4)); Fy_Matrix(4) * cosd(Tire_Angle(4)); Fz_Matrix(4) ];
    ...
    plot(Lat_Acc_Hold,Yaw_Acc_Hold)
    clear Yaw_Acc_Hold Lat_Acc_Hold
end

clearvars -except Lat_Acc_Hold Yaw_Acc_Hold
% 
