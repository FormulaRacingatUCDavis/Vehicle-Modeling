...
load("OpenTRACK_FSAE Skidpad_Closed_Forward.mat")
load("carParams.mat")

%%
v_samples = linspace(5, 70, 50);
...
parfor i = 1:length(r)
    max_speed_trace(i) = vehicle_model_lat(carParams, r(i), pp_Fy);
end
...
function v = vehicle_model_lat(carParams, r, pp_Fy)
     % speed solution
     if abs(r) < 1e-6 % At strait
         v = inf;
     else % Cornering
         r = abs(r);
...
function Fy = calcFy(v, carParams)
    rangeSA = [-5,5];
    rangeSteer = [-15,15];
    [SSAy, ~, ~] = MMD_Generattion(carParams, rangeSA, rangeSteer, v, false, 0);
