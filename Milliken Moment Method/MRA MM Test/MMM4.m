...
%% Inital Conditions & Initialization
delta = -120:5:120; %Change indexing back to normal  % Steer Angle [Deg]
Beta = -10:1:7; %change indexing back to normal    % Body Slip Angle [Deg]
...
for i = 1:length(delta)
    for j = 1:length(Beta)
        ...
        R_CG_Old = 15; %Initial Guess
         while abs(res) > tol & k < 10000
            ...
            SA(1) = Tire_Angle(1)-atan2d(V{1}(2),V{1}(1));
            SA(2) = Tire_Angle(2)-atan2d(V{2}(2),V{2}(1));
            SA(3) = Tire_Angle(3)-atan2d(V{3}(2),V{3}(1));
            SA(4) = Tire_Angle(4)-atan2d(V{4}(2),V{4}(1));
...
%% Graphing
hold on
dimensions = size(Lat_Acc_All);
for i = 1:dimensions(1)
    plot(Lat_Acc_All(i,:)./9.8,Yaw_Acc_All(i,:), 'b')
end
for i = 1:dimensions(2)
    plot(Lat_Acc_All(:,i)./9.8,Yaw_Acc_All(:,i), 'r')
end
clear Yaw_Acc_Hold Lat_Acc_Hold
...
clearvars -except Lat_Acc_Hold Lat_Acc_All Yaw_Acc_Hold Yaw_Acc_All R_CG_Hold