...
load Vehicle_information_test_0503_2017.mat
load Yaw_CS
%% Vehicle Parameters
...
sim('Vehicle_Dynamic_sim_V3_5')
%sim('Vehicle_Dynamic_sim')
...
hold on
plot(yaw_rate_nonlinear,'r');
hold on
plot(yaw_rate_simple,'g');
hold on
plot(yaw_rate_desired, 'b--');
title('Yaw Rate Comparison ');
xlabel('Time [s]');
ylabel('Yaw rate [deg/s]');
legend('CarSim','Ping-Linear','Ping-Nonlinear','Simple-Model','Ideal');
%ylim([-120 120]);
...
hold on
plot(normal_force_on_lefttire_front_NL,'r');
hold on
...
legend('Ping-Linear','Ping-Nonlinear','Simple-Model');
ylim([-7000 7000]);
%ylim([-30 30]);
%ratio = Fyf/Fyr;
%plot(t,ratio)