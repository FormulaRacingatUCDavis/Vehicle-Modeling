...
%     %     wr = (WR+DF*(1-CoP))/2;
%     % 
%     %     %%% REMOVED THE /24 FOR SOME REASON AND THE UNUSED FZ_vals = [-250:1:-50];
%     %     wf = wf-Ax*cg*WS/l/24;
%     %     wr = wr+Ax*cg*WS/l/24;
...
%     %     AX = FX/W;
%     %     AX_diff = AX-Ax;
%     %     AX_diff_save(itCount,i) = AX_diff;
%     % end
%     % A_xr(i) = AX;
%     % output = PowertrainlapsimV222(max(7.5,V/3.28)); % 7.5 reg, 10 launch
%     % FX = output(1)*.2248; % Converting from Newtons to lbf
%     % FX = FX-(1/2)*rho*crossA*Cd*V^2; %lbf
%     % fx(i) = FX/W; % 
%     % AX(i) = min(FX/W,A_xr(i));
%     % output = PowertrainlapsimV222(V/3.28);
%     % g = output(2);
%     % gear(i) = g;
%     % if g>gp
%     %     spcount = spcount+1;
...
clear latAccelSave lonAccelSave decAccelSave
velocity2 = [15:5:130]';
for i = 1:length(velocity2)
    latAccelSave(i,1) = fnval(cornering, velocity2(i));
    lonAccelSave(i,1) = fnval(accel, velocity2(i));
    decAccelSave(i,1) = fnval(deccel, velocity2(i));
end
...
subplot(2,3,1)
fnplt(accel)
title("Vx vs AXmax(power)")
subplot(2,3,2)
fnplt(grip)
title("Vx vs AXmax(grip)")
subplot(2,3,3)
fnplt(deccel)
title("Vx vs AXmax(braking)")
subplot(2,3,4)
fnplt(lateral)
title("Vy vs lateral Gs")
subplot(2,3,5)
fnplt(cornering)
title("turning radius vs Vy")

testV = 100; %ft/s
turningRadii = linspace(15,155,15); %ft
Vy_radii = fnval(cornering, turningRadii);
latGs_Vy = fnval(lateral, Vy_radii);

% Need somehow turning radius --> body slip angle WRONG: turning radii with
% relation to Vy already takes into account body slip angle
figure

testVy = Vy_radii;
testVx = sqrt(testV.^2 - Vy_radii.^2);
testAy = fnval(lateral, testVy);
testAx = min(fnval(accel, testVx), fnval(grip, testVx));
testBrake = fnval(deccel, testVx);
plot(testAx, testAy);
hold on
%plot(testBrake, testAy);
