function [out1,out2,out3] = ginput_mod(arg1, crosshair_color)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    initialState = setupFcn(fig, crosshair_color);
...
function initialState = setupFcn(fig, crosshair_color)
...
initialState.CrossHair = createCrossHair(fig, crosshair_color);
...
function crossHair = createCrossHair(fig, crosshair_color)
...
if isWebFigureType(fig, 'UIFigure')
    for k = 1:4
        crossHair(k) = uilabel(fig, 'Visible', 'off', 'BackgroundColor', crosshair_color, 'HandleVisibility', 'off'); %#ok<AGROW>
    end
else
    for k = 1:4
        crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'BackgroundColor', crosshair_color, 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
    end
end
